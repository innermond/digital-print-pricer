...
export function FinishingOptions({ element, config, onUpdate, badgeText }: FinishingOptionsProps) {
  ...
  const roundedCorners = allowedRoundedCorners(element, config);
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        {creasingCounts.length > 0 && (
          <CreasingControl
            ...
            // Spread the existing creasing object: it carries the catalog's `min`/
            // `max` alongside the chosen count, and rebuilding it would drop the
            // cap — which silently unmounts this control on the next render.
            onChange={(count) =>
