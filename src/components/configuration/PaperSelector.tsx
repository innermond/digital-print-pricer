...
export function PaperSelector({ papers, selectedId, recommendedId, onSelect, badgeText }: PaperSelectorProps) {
  const widget = (
    <div>
      ...
      <div className="flex flex-wrap gap-2">
        {papers.map((paper) => {
          ...
          return paper.explanation
            ? <Badge key={paper.id} text={paper.explanation}>{btn}</Badge>
            : btn;
        })}
      </div>
    </div>
